...
public class Gyroscope_Provider extends ContentProvider {
    ...
    public static String AUTHORITY = "io.github.cluo29.build.provider.gyroscope";
    ...
    public static final class Gyroscope_Data implements BaseColumns {
        ...
        public static final String CONTENT_TYPE = "vnd.android.cursor.dir/vnd.build.gyroscope.data";
        public static final String CONTENT_ITEM_TYPE = "vnd.android.cursor.item/vnd.build.gyroscope.data";
    ...
    public static final String[] DATABASE_TABLES = {"gyroscope" };
        ...
        switch (sUriMatcher.match(uri)) {
            case ACCEL_DATA:
                long accelData_id = database.insertWithOnConflict(DATABASE_TABLES[0],
                        Gyroscope_Data.TIMESTAMP, values, SQLiteDatabase.CONFLICT_IGNORE);
                if (accelData_id > 0) {
                    Uri accelDataUri = ContentUris.withAppendedId(
                            Gyroscope_Data.CONTENT_URI, accelData_id);
        ...
        switch (sUriMatcher.match(uri)) {
            case ACCEL_DATA:
                ...
                for (ContentValues v : values) {
                    long id;
                    try {
                        id = database.insertOrThrow(DATABASE_TABLES[0], Gyroscope_Data.TIMESTAMP, v);
                    } catch (SQLException e) {
                        id = database.replace(DATABASE_TABLES[0], Gyroscope_Data.TIMESTAMP, v);
                    }
                    if (id <= 0) {
    ...
    public boolean onCreate(){
        ...
        sUriMatcher.addURI(Gyroscope_Provider.AUTHORITY,
        ...
        accelDataMap.put(Gyroscope_Data._ID, Gyroscope_Data._ID);
        accelDataMap.put(Gyroscope_Data.TIMESTAMP,
                Gyroscope_Data.TIMESTAMP);
        accelDataMap.put(Gyroscope_Data.VALUES_0,
                Gyroscope_Data.VALUES_0);
        accelDataMap.put(Gyroscope_Data.VALUES_1,
                Gyroscope_Data.VALUES_1);
        accelDataMap.put(Gyroscope_Data.VALUES_2,
                Gyroscope_Data.VALUES_2);
        accelDataMap.put(Gyroscope_Data.ACCURACY,
                Gyroscope_Data.ACCURACY);
